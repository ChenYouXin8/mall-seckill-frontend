  ...
  <div class="page">
    <header class="page-head">
      <div>
        <div class="eyebrow-row">
          <p class="eyebrow page-eyebrow">ORDERS / 02</p>
          <span class="head-live"><i></i> LIVE MONITOR</span>
        </div>
        <h2 class="display-md page-title">订单管理</h2>
        <p class="page-sub">实时监控秒杀订单 · 支持手动关单并回补库存</p>
      </div>
      <div class="head-actions">
        <span v-if="activityFilter" class="filter-chip">场次 {{ activityFilter }}<button class="chip-close" @click="clearFilter">×</button></span>
    ...
    <section class="stats">
      <div class="stat stat--hero">
        <div class="stat-top"><span class="stat-label">ALL ORDERS</span><span class="stat-code">01</span></div>
        <span class="stat-num display-md">{{ totalOrders }}</span>
        <span class="stat-foot">当前列表订单</span>
      </div>
      <div class="stat">
        <div class="stat-top"><span class="stat-label">PENDING</span><span class="stat-code">02</span></div>
        <span class="stat-num display-md">{{ pendingOrders }}</span>
        <span class="stat-foot">等待支付</span>
      </div>
      <div class="stat stat--live">
        <div class="stat-top"><span class="stat-label">PAID</span><span class="signal"></span></div>
        <span class="stat-num display-md">{{ paidOrders }}</span>
        <span class="stat-foot">已完成支付</span>
      </div>
      <div class="stat">
        <div class="stat-top"><span class="stat-label">CLOSED</span><span class="stat-code">04</span></div>
        <span class="stat-num display-md">{{ closedOrders }}</span>
        <span class="stat-foot">已取消 / 已关闭</span>
      </div>
    </section>

    <section class="table-card">
      <div class="table-head">
        <div><p class="section-kicker">ORDER STREAM</p><h3>订单流水</h3></div>
        <span class="table-count">AUTO REFRESH · 30S</span>
      </div>
      <div class="table-wrap">
        <el-table :data="orders" v-loading="loading" class="table" empty-text="暂无订单数据">
          <el-table-column prop="orderNo" label="订单号" min-width="205" show-overflow-tooltip>
            <template #default="{ row }"><span class="mono order-no">{{ row.orderNo }}</span></template>
          </el-table-column>
          <el-table-column prop="userId" label="用户" width="90">
            <template #default="{ row }"><span class="mono muted-id">U{{ row.userId }}</span></template>
          </el-table-column>
          <el-table-column prop="activityId" label="场次" width="92" show-overflow-tooltip>
            <template #default="{ row }"><span class="mono muted-id">{{ row.activityId }}</span></template>
          </el-table-column>
          <el-table-column prop="productId" label="商品" width="82">
          ...
          <el-table-column label="状态" width="112">
            <template #default="{ row }"><span class="status-pill" :class="statusClass(row.status)"><i></i>{{ statusText(row.status) }}</span></template>
          </el-table-column>
          <el-table-column label="创建时间" width="145">
            <template #default="{ row }"><span class="mono cell-time">{{ formatTime(row.createdAt) }}</span></template>
          </el-table-column>
          <el-table-column label="过期时间" width="145">
            <template #default="{ row }"><span class="mono cell-time">{{ formatTime(row.expireTime) }}</span></template>
          </el-table-column>
          <el-table-column label="操作" width="104" fixed="right">
            <template #default="{ row }">
              <el-button v-if="row.status === 0" size="small" type="danger" plain @click="closeOrder(row)">关单</el-button>
    ...
    <div class="pagination">
      <el-pagination background layout="total, prev, pager, next" :total="totalOrders" v-model:current-page="currentPage" :page-size="20" @current-change="loadOrders" />
    </div>
  </div>
</template>
...
function formatTime(timeStr) { if (!timeStr) return '-'; return dayjs(timeStr).format('MM-DD HH:mm:ss') }
function formatPrice(p) { if (p === null || p === undefined || p === '') return '-'; const n = Number(p); return Number.isNaN(n) ? '-' : n.toFixed(2) }
function statusClass(status) { return { 0: 'status--pending', 1: 'status--paid', 2: 'status--closed', 3: 'status--closed', 4: 'status--refunded' }[status] || 'status--closed' }
...
async function loadOrders() {
  ...
  try {
    ...
    let list = res.data || []
    if (activityFilter.value) list = list.filter(o => String(o.activityId) === String(activityFilter.value))
    orders.value = list
  } catch {
...
onMounted(() => { loadOrders(); statusTimer = setInterval(loadOrders, 30000) })
...
.filter-chip { display:inline-flex; align-items:center; gap:9px; min-height:32px; padding:0 7px 0 12px; border-radius:99px; background:#e9f2e5; color:#557351; font-size:11px; }
.chip-close { width:18px; height:18px; border:0; border-radius:50%; background:rgba(69,125,77,.12); color:#557351; cursor:pointer; line-height:1; }
...
.table { width:100%; }
.order-no { font-size:10px; color:#55554f; }
.muted-id { font-size:9px; color:#88877f; }
...
.cell-time { font-size:10px; color:var(--shade-40); }
.status-pill { display:inline-flex; align-items:center; gap:6px; padding:6px 9px; border-radius:99px; font-size:10px; font-weight:560; white-space:nowrap; }
.status-pill i { width:5px; height:5px; border-radius:50%; background:currentColor; }
.status--pending { color:#85847c; background:#f0eee8; }
.status--paid { color:#457d4d; background:#e4f1e1; }
.status--closed { color:#96958e; background:#f2f1ed; }
.status--refunded { color:#787771; background:#e8e7e2; }
.muted-dash { color:#b9b8b1; font-size:12px; }
...
@media (max-width:760px) { .page-head { align-items:flex-start; flex-direction:column; } .head-actions { width:100%; } .head-actions .el-button,.filter-chip { flex:1; } .stats { grid-template-columns:1fr 1fr; } }
</style>
